...
const ChatBox = () => {
  ...
  const onSubmit = async (e) => {
    try 
    {
      ...
      const {data} = await axios.post(`api/messages/${mode}`, {chatId: selectedChats._id, prompt, isPublished}, {headers: {Authorization: token}});
    ...
    <main className='flex-1 flex flex-col justify-between m-5 md:m-10 xl:mx-30 max-md:mt-14 2xl:pr-40'>
      ...
      {mode === 'image' && (
        <label className='inline-flex items-center gap-2 mb-3 text-sm mx-auto'>
          <p className='text-xs cursor-pointer'>Publish Generated Image to Community..!</p>
      ...
      <form onSubmit={onSubmit} className='bg-primary/20 dark:bg-[#583C79]/30 border border-primary dark:border-[#80609F]/30 rounded-full w-full max-w-2xl p-3 pl-4 mx-auto flex gap-4 items-center'>
        ...
        <input  onChange={ (e) => setPrompt(e.target.value) } value={prompt} type="text"  placeholder={ mode === 'text' ? "Type your prompt here..." : "Describe the image you want..." }  className='flex-1 w-full text-sm outline-none' required />
        
        <button disabled={loading}>
          <img src={loading ? assets.stop_icon : assets.send_icon} alt="stop_send_icon" className='w-8 cursor-pointer' />
        </button>
      </form>
