...
const Sidebar = ({ isMenuOpen, setIsMenuOpen }) => {
  ...
  const { chats, setSelectedChats, theme, setTheme, user, navigate, createNewChat, axios, setChats, fetchUserChats, setToken, token } = useAppContext();
  ...
  return (
    <div className={`flex flex-col h-screen min-w-72 p-5 dark:bg-gradient-to-b from-[#242124]/30 to-[#000000]/30 border-r border-[#80609F]/30 backdrop-blur-3xl transition-all duration-500 max-md:absolute left-0 z-1 ${!isMenuOpen && 'max-md:-translate-x-full'}`}>
      ...
      <div className='flex items-center gap-2 p-3 mt-4 border border-gray-400 dark:border-white/20 rounded-md'>
        <img src={assets.search_icon} alt="search_icon" className='w-4 not-dark:invert' />
      ...
      <div className='flex-1 overflow-y-scroll mt-3 text-sm space-y-3'> 
        {
          chats
            ?.filter((chat) => chat && chat._id)   // khali valid chats filter karva
            .filter((chat) => {
            ...
            .map((chat) => (
              <div
                ...
                <img
                  ...
                  src={assets.bin_icon}
                  className="hidden group-hover:block w-4 cursor-pointer not-dark:invert" alt="bin_icon" />
              </div>
            ))
        }
      </div>
      ...
      <div onClick={ () => {navigate('/community'); setIsMenuOpen(false);} } className='flex items-center gap-2 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer hover:scale-103 transition-all'>
        <img src={assets.gallery_icon} alt="gallery_icon" className='w-4.5 not-dark:invert'/>
      ...
      <div onClick={() => { navigate('/thirdparty'); setIsMenuOpen(false);} } className='flex items-center gap-2 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer hover:scale-103 transition-all'>
      ...
      <div className='flex items-center justify-between gap-2 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md'>
        <div className='flex items-center gap-2 text-sm'>
          <img src={assets.theme_icon} alt="theme_icon" className='w-4 not-dark:invert' />
          <p>Dark Mode</p>
        </div>
      ...
      <div className='flex items-center gap-3 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer group'>
        ...
        { user && <img onClick={logout} src={assets.logout_icon} alt="logout_icon" className='h-5 cursor-pointer hidden not-dark:invert group-hover:block' /> }
      </div>

      {/* responsiveness */}
      <img onClick={ () => setIsMenuOpen(false) } src={assets.close_icon} alt="close_icon" className='absolute top-3 right-3 w-5 h-5 cursor-pointer md:hidden not-dark:invert' />
